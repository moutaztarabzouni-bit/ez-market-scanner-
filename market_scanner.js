// ╔══════════════════════════════════════════════════════════════════════╗
// ║  PROFESSIONAL MARKET SCANNER — LOCKED VERSION v5.0                 ║
...
const FIXES_MANIFEST = Object.freeze({
  version:    '5.0',
  locked:     '2026-04-08',
  checksums:  Object.freeze({"getFhKey": "b5a179c4", "claudeCall": "32022f29", "fetchT": "e6719b2d", "safeCall": "a889e8f4", "circuitOpen": "93288e39", "runWatchdog": "becf406b", "startWatchdog": "314f57f9", "loadSectorHeatmap": "2cdb0186", "fetchFinnhubPrices": "51292335", "fetchFinnhubIndices": "d517ecda", "fetchLiquidityData": "b9473bf7", "fetchAndRenderScanNews": "aaa03ffe", "detectContradiction": "f8c292f9", "renderScanNewsSection": "6b8054d2", "renderLiquidityDataAnalysis": "dce0a8f7", "computeLiquidityScore": "8a5a2c24", "computeHealthScore": "b4305512", "computeDynamicSignal": "5034ad30", "computeEntryLevels": "2bc58804", "applyLivePrices": "6bd08800", "finalize": "a915b631", "runScan": "7daca50c", "autoAudit": "23104a5e", "runAudit": "96ed2a0b", "buildChatContext": "86ee12f4", "processResultsWithWorker": "40457cbe", "runIntegrityCheck": "b1aa0fca"}),
...
// Auto-run checksum verification 3 seconds after load
setTimeout(() => {
  try{ manifestCheck(); }catch(e){}
}, 3000);
...
function runIntegrityCheck(){
  ...
  var MANIFEST_FNS = [
    ...
    'saveLastScan','loadLastScan','toggleTheme','toggleLang',
    'sendChat','buildChatContext','updateChatContextBar','appendMsg',
    'showThinking','hideThinking','formatResponse','chatSuggest','updateChatSuggestions','clearChat',
  ...
  var MANIFEST_IDS = [
    ...
    'chat-messages','chat-input','chat-send-btn',
    'ar-timer','theme-btn','lang-btn',
  ...
  var tabs_ok      = ['p1','p2','p3','p4','p5','p6','p7','p8','p9'].every(function(id){return !!document.getElementById(id);});
  ...
  if(report.healthy){
    console.log(
      ...
      `[OK] Constants locked . [OK] All 9 tabs`,
...
window.featureList = function(){
  var cats = {
    ...
    '? Chat':          ['sendChat','buildChatContext'],
    '[LIQ] Liquidity':     ['runLiquidityAnalysis','computeLiquidityScore'],
    '[ELITE] Elite':         ['runEliteEngine','renderEliteOutput','addEliteToPortfolio'],
...
function lockAnthropicUI(){
  ...
  if(banner){
    ...
    <div id="api-saved" style="color:var(--green);display:block;margin-top:6px;font-size:11px;font-weight:600">OK Anthropic key active - AI Chat . New stocks . Live news enabled</div>`;
  }
  updateApiKeyBtn();
}
function getKey(){
...
// ==============================================
// ? AI CHAT - Claude with full scan context
// ==============================================
const chatHistory=[];
let chatThinking=false;

function buildChatContext(){
  if(!results.length)return'No stocks have been scanned yet.';
  const liqHist=JSON.parse(localStorage.getItem('mkt_liq_history')||'[]');
  const liqStr=liqHist.length?` | Liquidity: ${liqHist[0].score}/100 VIX:${liqHist[0].vix??'-'}`:'';
  const tsStr=scanTimestamp?` | Scanned: ${new Date(scanTimestamp).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'})}`:'';
  const ctx='Current scan #'+scanSessionId+tsStr+': '+results.length+' US stocks. Market: '+(document.getElementById('m-sp')?.textContent||'-')+' S&P500'+liqStr+'.\n\nSCANNED STOCKS:\n'+
    results.map(r=>'- '+r.t+' ('+r.n+'): '+(SLB[r.sig]||r.sig).toUpperCase()+' | Health '+r.hp+'% | $'+(r.price?.toFixed(2)||'-')+' ('+(r.chg||0>=0?'+':'')+(r.chg||0).toFixed(2)+'%) | RSI '+(r.rsi||'-')+' | '+(r.macd||'-')+' | '+(r.ma||'-')+(r.entry>0?' | Entry $'+r.entry.toFixed(2)+' TP $'+r.tp.toFixed(2)+' R/R '+r.rr+':1':'')+(r.thesis&&r.thesis!=='-'?' | '+r.thesis.slice(0,120):'')+(r.analyst_consensus?' | Analyst: '+r.analyst_consensus+(r.price_target?' PT$'+r.price_target:''):'') ).join('\n')+
    '\n\nPortfolio: '+(portfolio.length>0?portfolio.map(p=>p.t+' '+p.qty+'sh@$'+p.buy).join(', '):'None.');
  return ctx;
}

function updateChatContextBar(){
  const el=document.getElementById('chat-ctx-stocks');if(!el)return;
  if(!results.length){el.textContent='No stocks loaded - run a scan first';return;}
  el.innerHTML=results.slice(0,8).map(r=>'<span class="ctx-tag">'+r.t+'</span>').join(' ')+(results.length>8?' <span style="color:var(--text3)">+'+( results.length-8)+' more</span>':'');
}

function appendMsg(role,html){
  const c=document.getElementById('chat-messages');if(!c)return;
  const d=document.createElement('div');d.className='msg '+role;
  d.innerHTML='<div class="msg-avatar">'+(role==='user'?'[USER]':'[BOT]')+'</div><div class="msg-bubble">'+html+'</div>';
  c.appendChild(d);c.scrollTop=c.scrollHeight;
}

function showThinking(){
  const c=document.getElementById('chat-messages');if(!c)return;
  const d=document.createElement('div');d.className='msg assistant';d.id='chat-thinking';
  d.innerHTML='<div class="msg-avatar">[BOT]</div><div class="chat-thinking"><div class="chat-dot"></div><div class="chat-dot"></div><div class="chat-dot"></div><span style="margin-left:4px">Analyzing...</span></div>';
  c.appendChild(d);c.scrollTop=c.scrollHeight;
}
...
async function sendChat(){
  ...
  if(!userMsg||chatThinking)return;
  inp.value='';inp.style.height='42px';
  chatThinking=true;btn.disabled=true;btn.textContent='...';
  appendMsg('user',userMsg.replace(/</g,'&lt;'));
  chatHistory.push({role:'user',content:userMsg});
  showThinking();

  const ctx=buildChatContext();
  const sysPmt='You are an expert US equity analyst embedded in a professional market scanner. You have full real-time context on the scanned portfolio.\n\n'+ctx+'\n\nRULES:\n- Answer in the user\'s language (Arabic or English)\n- Be concise but institutional-grade - cite actual numbers from the scan\n- Use **bold** for key figures, tickers, and signals\n- When asked about specific stocks, reference their exact health score, RSI, entry/target levels\n- Keep answers under 400 words unless a detailed breakdown is requested\n- Today: '+new Date().toLocaleDateString('en-US',{weekday:'long',year:'numeric',month:'long',day:'numeric'});

  const msgs=[...chatHistory.slice(-8).slice(0,-1),{role:'user',content:userMsg}];

  try{
    const data=await claudeCall(msgs,sysPmt,900);
    const reply=(data.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('').trim();
    if(!reply)throw new Error('Empty response');
    hideThinking();
    appendMsg('assistant',formatResponse(reply));
    ...
    if(msg==='NO_KEY'){
      appendMsg('assistant','<span style="color:var(--amber)">[WARN] Add Anthropic API key above to enable AI Chat.<br><small style="color:var(--text3)">Add Anthropic API key in the banner above, then click Save Key.</small></span>');
      document.querySelector('.api-banner')?.scrollIntoView({behavior:'smooth',block:'start'});
    } else {
      const hint=msg.includes('401')||msg.includes('auth')?'<br><small>Check your API key.</small>':msg.includes('timeout')||msg.includes('abort')?'<br><small>Timeout - please try again.</small>':'';
      appendMsg('assistant','<span style="color:var(--red)">[WARN] '+msg+hint+'</span>');
    }
  }
  chatThinking=false;btn.disabled=false;btn.textContent='Send >';
}
...
function updateChatSuggestions(){
  ...
  if(!el||el.dataset.updated)return;
  const buys=results.filter(r=>r.sig==='buy').slice(0,2).map(r=>'<button class="chat-sug" onclick="chatSuggest(\'Give me a detailed analysis of '+r.t+'\')">? '+r.t+' deep dive</button>').join('');
  const sell=results.find(r=>r.sig==='sell');
  const sellQ=sell?'<button class="chat-sug" onclick="chatSuggest(\'Why should I avoid '+sell.t+'?\')">? Why avoid '+sell.t+'?</button>':'';
  el.innerHTML+=buys+sellQ;
...
function clearChat(){
  ...
  if(c)c.innerHTML='<div class="msg assistant"><div class="msg-avatar">[BOT]</div><div class="msg-bubble"><strong>Chat cleared.</strong> Ask me anything about the current scan.</div></div>';
}
...
function liqRegime(score){
  ...
  return { label:'? Stressed / Illiquid', cls:'stressed' };
}
...
function addToWatchlist(){
  const t=document.getElementById('watch-ticker-inp').value.trim().toUpperCase();
  const entry=+document.getElementById('watch-entry-inp').value;
  const target=+document.getElementById('watch-target-inp').value;
  const stop=+document.getElementById('watch-stop-inp').value;
  const note=document.getElementById('watch-note-inp').value.trim();
  if(!t||!target)return showAlertBanner('Ticker and Target are required','var(--amber)');
  if(watchlist.find(w=>w.t===t)){showAlertBanner(`${t} already in watchlist`,'var(--blue)');return;}
  watchlist.unshift({t,entry:entry||0,target,stop:stop||0,note,addedAt:Date.now(),price:0,priceTs:0});
  saveWatchlist();
...
function renderWatchlist(){
  ...
  if(!body)return;
  if(!watchlist.length){body.innerHTML='<div style="font-size:12px;color:var(--text3);padding:1rem 0;text-align:center">No stocks in watchlist - add one above</div>';return;}
...
function addEliteToPortfolio(ticker,entry,stop,target,category){
  ...
  const price=results.find(r=>r.t===ticker)?.price||entry;
  portfolio.unshift({t:ticker,qty:1,buy:entry||price,price,sl:stop,tp:target,note:`Elite Engine . ${category}`,addedAt:Date.now()});
  localStorage.setItem('mkt_portfolio',JSON.stringify(portfolio));
...
async function runAudit(){
  ...
  const items = [
    ...
    { icon:'⬛', label:'All 9 Tabs',      value: r.tabs_ok?'All present':'Missing!', pass: r.tabs_ok },
    ...
    { icon:'💬', label:'AI Chat',         value: typeof sendChat==='function'&&typeof buildChatContext==='function'?'OK':'FAIL', pass: typeof sendChat==='function' },
  ...
  if(trigBtn){
    ...
    trigBtn.innerHTML = `<span class="audit-trigger-dot"></span>${allOk ? 'All OK' : failed.length + ' Issues'}`;
  }
...
async function autoAudit(){
  ...
  if(trigBtn){
    trigBtn.className = 'audit-trigger-btn ' + (allOk ? 'ok' : 'fail');
    trigBtn.innerHTML = `<span class="audit-trigger-dot"></span>${allOk ? 'All OK' : 'Issues Found'}`;
  }
...
window.restore = function(what = 'all'){
  const actions = {
    ...
    'chat': function(){
      clearChat();
      console.log('✅ Chat cleared and ready');
    },
...
window.addEventListener('load',()=>{
  ...
  // Language
  const lang=localStorage.getItem('mkt_lang');
  if(lang==='rtl'){document.documentElement.dir='rtl';document.documentElement.lang='ar';const b=document.getElementById('lang-btn');if(b)b.textContent='EN';}
  // Panels
...
function setMode(m){
  ...
  if(m===6){updateChatContextBar();updateChatSuggestions();}
...
async function runScan(){
  ...
  setSts('Scanning...','on');results=[];openRow=null;showS(false);setP('s',0,'');
  newsLRU.clear();compareSet.clear();updateCompareBar();

  const allTickers=Object.keys(DB);
  const now=new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'});
  setP('s',3,'Launching parallel data fetch...');
  setYahooBar('loading','Connecting to Finnhub...','','');
  setPriceIndicator('loading','Fetching all data in parallel...');

  // ─── PHASE 1: All external data IN PARALLEL ───────────────────────────────
  // Prices + Indices + Sector Heatmap + Liquidity raw data — simultaneously
  setP('s',8,'Fetching: Prices + Indices + Sectors + Liquidity...');

  let liveMap={}, idxMap=null, liqData=null;

  const [priceResult, idxResult, liqResult] = await Promise.allSettled([
    // 1. Live prices for all 25 DB stocks
    fetchFinnhubPrices(allTickers),
    // 2. Market indices (SPY, QQQ, VIX, Oil, Gold)
    fetchFinnhubIndices(),
    // 3. Liquidity raw data (VXX, SPY, HYG, IWM)
    fetchLiquidityData(),
    // NOTE: heatmap runs separately after prices, to avoid Finnhub rate limits
  ]);

  // ─── Process prices ───────────────────────────────────────────────────────
  setP('s',35,'Processing prices...');
  if(priceResult.status==='fulfilled'&&Object.keys(priceResult.value||{}).length>0){
    liveMap=priceResult.value;
    const liveCount=Object.keys(liveMap).length;
    const t=new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'});
    setYahooBar('ok','Finnhub OK - '+liveCount+' live prices',liveCount+'/'+allTickers.length+' stocks','as of '+t);
    setPriceIndicator('live','Live - '+liveCount+'/'+allTickers.length+' stocks',t);
  } else {
    // Plan B: web search for prices
    if(getKey()){
      setP('s',20,'Plan B: Web search prices...');
      ...
      try{
        ...
        const t=new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'});
        setYahooBar('ok','Plan B OK - '+lc+' via web search',lc+'/'+allTickers.length,'as of '+t);
        setPriceIndicator('live','Live (Web) - '+lc+'/'+allTickers.length,t);
      }catch(e2){
  ...
  if(liqResult.status==='fulfilled'&&liqResult.value){
    liqData=liqResult.value;
    const score=computeLiquidityScore(liqData);
    const regime=liqRegime(score);
    // Update Liquidity tab live
    ...
    // Show liquidity score in market bar
    const liqBarEl=document.getElementById('m-liq');
    if(liqBarEl){
      liqBarEl.textContent=score+'/100';
      liqBarEl.style.color=liqColor(score);
    }
    // Update scan summary bar liquidity
    ...
    // Step A: Instant data-based analysis (no Claude needed)
    renderLiquidityDataAnalysis(liqData, score, regime);
    // Step B: Try Claude deep analysis in background
    if(getKey()){
  ...
  // ─── Sector Heatmap: runs after prices settle (avoids rate limit collision) ──
  setTimeout(() => loadSectorHeatmap().catch(() => {}), 600);

  // ─── PHASE 2: Build results from DB + live prices ────────────────────────
  setP('s',55,'Computing health scores...');
  const list=Object.entries(DB).map(([t,d])=>({t,...d}));
  list.forEach(item=>results.push(applyLivePrices(item,liveMap)));
  results.sort((a,b)=>b.hp-a.hp);

  // ─── PHASE 3: Finalize + render ──────────────────────────────────────────
  setP('s',85,'Rendering results...');
  await finalize('scan');

  // ─── PHASE 4: Post-render tasks (parallel, non-blocking) ─────────────────
  setP('s',100,'Complete');
  Promise.allSettled([
    // Market Intelligence check
    checkMarketIntelligence(),
    // News is handled by fetchAndRenderScanNews in finalize
    // Save to IDB
    saveScanToIDB(results,'scan'),
  ]).catch(()=>{});

  btn.disabled=false;btn.textContent='Scan Market';
  setSts('Live - '+results.length+' stocks','done');
}
...
function buildDetail(r,i){
  ...
  <div class="dtabs" id="DT${i}">
    ...
    <button class="dtab" onclick="swTab(${i},'t')">[CHART] Technical</button>
...
function showS(v){document.getElementById('shared').style.display=v?'':'none';}
function fillMetrics(data,m){
  ...
  document.getElementById('cnt2').textContent=m==='scan'?'from database':'stocks analyzed';
...
// ?  PRICE ALERTS
// ==============================================
let alerts=JSON.parse(localStorage.getItem('mkt_alerts')||'[]');
function addAlert(){
  const t=document.getElementById('alert-ticker').value.toUpperCase().trim();
  const p=parseFloat(document.getElementById('alert-price').value);
  const d=document.getElementById('alert-dir').value;
  if(!t||!p)return;
  alerts.push({t,p,d,active:true,created:new Date().toLocaleDateString('en-US')});
  localStorage.setItem('mkt_alerts',JSON.stringify(alerts));
  document.getElementById('alert-ticker').value='';
  document.getElementById('alert-price').value='';
  renderAlerts();
}
function removeAlert(idx){alerts.splice(idx,1);localStorage.setItem('mkt_alerts',JSON.stringify(alerts));renderAlerts();}
...
function checkAlerts(liveMap){
  ...
  alerts.forEach((a,idx)=>{
    ...
    const hit=a.d==='above'?live.price>=a.p:live.price<=a.p;
    if(hit){alerts[idx].active=false;changed=true;fireNotification(a,live.price);}
  });
  if(changed){localStorage.setItem('mkt_alerts',JSON.stringify(alerts));if(mode===4)renderAlerts();}
}
function fireNotification(a,currentPrice){
  const msg=`${a.t} is ${a.d==='above'?'above':'below'} $${a.p} - Now: $${currentPrice.toFixed(2)}`;
...
function showAlertBanner(msg,bg='var(--amber)'){
  const d=document.createElement('div');d.className='alert-banner';d.style.background=bg;d.textContent=msg;
  document.body.appendChild(d);setTimeout(()=>d.remove(),5000);
}
function renderAlerts(){
  const el=document.getElementById('alerts-body');if(!el)return;
  if(!alerts.length){el.innerHTML='<div class="empty">No alerts set</div>';return;}
...
// ?  PORTFOLIO TRACKER
// ==============================================
let portfolio=JSON.parse(localStorage.getItem('mkt_portfolio')||'[]');
function addPosition(){
  const t=document.getElementById('pos-ticker').value.toUpperCase().trim();
  const qty=parseFloat(document.getElementById('pos-qty').value);
  const buy=parseFloat(document.getElementById('pos-buy').value);
  const date=document.getElementById('pos-date').value;
  if(!t||!qty||!buy)return;
  portfolio.push({t,qty,buy,date,n:DB[t]?.n||t});
  localStorage.setItem('mkt_portfolio',JSON.stringify(portfolio));
  document.getElementById('pos-ticker').value='';document.getElementById('pos-qty').value='';document.getElementById('pos-buy').value='';
  refreshPortfolio();
}
function removePosition(idx){portfolio.splice(idx,1);localStorage.setItem('mkt_portfolio',JSON.stringify(portfolio));refreshPortfolio();}
async function refreshPortfolio(){
  const el=document.getElementById('portfolio-body');if(!el)return;
  if(!portfolio.length){el.innerHTML='<div class="empty">No positions yet - add your first stock above</div>';return;}
...
function renderHistory(){
  ...
  el.innerHTML=scanHistory.map(h=>`
    <div class="hist-item">
      <div style="flex:1">
        <div style="font-size:12px;font-weight:600;color:var(--text)">${h.mode==='scan'?'[CHART] Full Market Scan':'? Custom Analysis'} . ${h.count} stocks</div>
...
function toggleTheme(){
  ...
  localStorage.setItem('mkt_theme',isLight?'light':'dark');
...
function toggleLang(){
  ...
  html.dir=isRTL?'ltr':'rtl';html.lang=isRTL?'en':'ar';
  localStorage.setItem('mkt_lang',isRTL?'ltr':'rtl');
